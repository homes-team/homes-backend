...
public interface AdminVerificationControllerDocs {
    @Operation(summary = "집주인 서류 수동 검수 대기 목록 조회", description = "자동 인증에 실패하여 관리자의 수동 검수가 필요한 집주인 인증 내역을 조회합니다.")
    ApiResponse<List<AdminOwnerVerificationListResDto>> getManualReviewList();

    @Operation(summary = "집주인 서류 수동 검수 결과 처리", description = "관리자가 서류를 확인하고 승인(true) 또는 반려(false) 처리합니다.")

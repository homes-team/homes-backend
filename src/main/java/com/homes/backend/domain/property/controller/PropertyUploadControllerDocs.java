...
public interface PropertyUploadControllerDocs {
    ...
    @Operation(summary = "이미지 업로드용 Presigned URL 발급", description = "사용자가 집 사진을 올릴 때 서버 부하를 줄이기 위해, " +
            ...
            "로그인한 상태라면 JWT를 그대로 보내고(매물 사진용), 아직 계정이 없는 중개사 가입 지원자는 " +
            "이메일 인증을 마친 email을 쿼리파라미터로 함께 보내야 발급이 됩니다(서류 사진용). " +
            "이 발급 주체 정보는 나중에 실제 제출(회원가입/매물등록) 시 대조되며, 한 번 제출에 성공하면 재사용할 수 없습니다.")
    ApiResponse<PresignedUrlResDto> getPresignedUrl(
            ...
            @Parameter(description = "이메일 인증을 마친 이메일 (로그인 전 중개사 가입 지원자만 필요, 로그인 상태면 생략)", example = "realtor@homes.com")
